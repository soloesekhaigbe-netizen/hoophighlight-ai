...
// Glass fixture archive.
export default function PortfolioGameList({ games }) {
  if (!games?.length) {
    return (
      <div>
        <h2 className="display-xl text-5xl sm:text-7xl">Games.</h2>
        <div className="mt-8 glass squircle-lg border border-dashed border-white/15 p-10 text-center sm:p-16">
  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-6 border-b border-white/10 pb-4">
        <h2 className="display-xl text-5xl sm:text-7xl">Games.</h2>
        <span className="label-xs text-foreground/50">{games.length} fixture(s)</span>
      </div>

      <ul className="mt-6 glass squircle-lg divide-y divide-white/10">
        {games.map((g, i) => {
          const d = g.game_date ? new Date(g.game_date) : null;
          return (
            <li key={g.id} className={`grid items-center gap-3 px-5 py-6 sm:grid-cols-12 ${i % 2 ? "bg-white/[0.03]" : ""}`}>
              <div className="flex items-baseline gap-4 sm:col-span-3">
                {d && (
                  <div className="leading-none">
                    ...
                    <p className="label-xs text-foreground/60">{d.toLocaleDateString(undefined, { month: "short" })} {d.getFullYear()}</p>
                  </div>
                )}
              </div>
              <div className="sm:col-span-7">
                <p className="font-display text-2xl uppercase sm:text-3xl">{g.name}</p>
                <p className="mt-1 text-sm text-foreground/70">{g.opponent ? `vs ${g.opponent}` : "Scrimmage / fixture"}</p>
              </div>
              <div className="sm:col-span-2 sm:justify-self-end">
                <span className="label-xs inline-flex items-center gap-1.5 text-foreground/60">
