  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-3xl border border-white/5 bg-white/[0.03] p-6">
        <p className="text-[11px] tracking-[0.24em] text-slate-500">PORTFOLIO CONTENT</p>

        {/* Profile photo */}
        <div className="mt-5 flex items-center gap-4">
          <div className="relative h-28 w-24 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-black/30">
            {project.profile_photo ? (
              <Image src={project.profile_photo} alt={project.player_name} fittingType="fill" className="h-full w-full" />
            ) : (
              <div className="flex h-full w-full items-center justify-center text-slate-600"><Upload className="h-6 w-6" /></div>
            )}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-medium">Profile picture</p>
            <p className="text-xs text-slate-500">Shown at the top of your portfolio.</p>
            <div className="mt-2 flex flex-wrap gap-2">
              <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-orange-500 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-orange-400">
                {uploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}
                {uploading ? "Uploading…" : "Upload"}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => uploadPhoto(e.target.files?.[0])} />
              </label>
              {project.profile_photo && (
                <Button size="sm" variant="ghost" className="text-slate-400 hover:text-rose-300" onClick={() => patch({ profile_photo: "" })}>
        ...
        <div className="mt-6">
          ...
          <p className="mt-1 text-[11px] text-slate-500">Saved automatically when you click away.</p>
        </div>

        {/* Featured highlight reel */}
        <div className="mt-6">
          <Label className="text-xs text-slate-400">Featured highlight reel</Label>
          {reels.length === 0 ? (
            <p className="mt-2 rounded-xl border border-dashed border-white/10 p-4 text-xs text-slate-500">
          ...
          <p className="mt-1 text-[11px] text-slate-500">The featured reel plays first when a coach opens your link.</p>
        </div>
      </div>
        ...
        <div className="rounded-3xl border border-white/5 bg-white/[0.03] p-6">
          ...
          <div className="mt-6 space-y-3">
            <label className="flex items-center justify-between rounded-xl border border-white/10 px-4 py-3">
              ...
              <input type="checkbox" checked={project.is_public !== false} onChange={(e) => patch({ is_public: e.target.checked })} className="h-4 w-4 accent-orange-500" />
            </label>
            <label className="flex items-center justify-between rounded-xl border border-white/10 px-4 py-3">
              <span className="text-sm">Show my email to coaches</span>
              <input type="checkbox" checked={project.show_email === true} onChange={(e) => patch({ show_email: e.target.checked })} className="h-4 w-4 accent-orange-500" />
            </label>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-white/5 bg-white/[0.03] p-6">
          ...
          <ul className="mt-4 space-y-2 text-sm text-slate-300">
            {[
              ...
              <li key={x.t} className="flex items-center gap-2"><x.icon className="h-4 w-4 text-orange-400" /> {x.t}</li>
            ))}
          </ul>
