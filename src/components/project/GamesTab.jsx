    ...
    <div className="space-y-5">
      <div className="flex justify-end">
        <AddGameDialog projectId={project.id} onDone={reload}
          trigger={<Button className=""><Plus className="mr-2 h-4 w-4" />ADD GAME</Button>} />
      </div>
      ...
      {games.length === 0 && (
        <div className="glass squircle-lg border-dashed border-white/15 p-14 text-center">
          ...
          <p className="mt-1 text-sm text-foreground/55">Upload footage or add a Veo/YouTube link — automatic analysis will find your highlights.</p>
          <AddGameDialog projectId={project.id} onDone={reload}
            trigger={<Button className="mt-5 "><Plus className="mr-2 h-4 w-4" />UPLOAD GAME</Button>} />
        </div>
      )}
      ...
      {games.map((game) => {
        ...
        return (
          <div key={game.id} className="glass squircle-lg p-6 transition hover:bg-white/10">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <button onClick={() => navigate(`/project/${project.id}/game/${game.id}`)}
                className="group min-w-0 text-left">
                <div className="flex items-center gap-2">
                  <p className="font-heading text-lg font-semibold group-hover:text-primary">{game.name}</p>
                  <ChevronRight className="h-4 w-4 text-foreground/30 transition group-hover:text-primary" />
                </div>
            ...
            <div className="mt-4 space-y-3">
              {gs.map((s) => (
                <div key={s.id} className="glass squircle p-4">
                  ...
                  {s.source_type !== "file" && s.status === "ready" && (s.clips_detected || 0) === 0 && (
                    <p className="mt-3 flex gap-2 squircle-sm border border-sky-500/25 bg-sky-500/10 p-3 text-xs text-sky-200">
