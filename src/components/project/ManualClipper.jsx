...
// Shared row: live timestamp + Mark In / Mark Out buttons.
function MarkerBar({ current, duration, onIn, onOut }) {
  return (
...
function FileMarker({ src, setStart, setEnd }) {
  ...
  return (
    <>
      ...
      <MarkerBar current={current} duration={duration}
        onIn={() => setStart(t())} onOut={() => setEnd(t())} />
    </>
  );
}
...
function YouTubeMarker({ videoId, setStart, setEnd }) {
    ...
    loadYouTubeAPI().then((YT) => {
      ...
      poll = setInterval(() => {
        try {
          ...
          if (p && p.getCurrentTime) setCurrent(p.getCurrentTime() || 0);
          if (p && p.getDuration) setDuration(p.getDuration() || 0);
        } catch (_e) {}
      }, 250);
    });
    return () => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl border-white/10 bg-slate-950 text-slate-100">
        ...
        <div className="space-y-4">
          ...
          <Button onClick={save} disabled={!form.video_source_id}
