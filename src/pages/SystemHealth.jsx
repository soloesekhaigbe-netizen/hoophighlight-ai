    ...
    <div className="mx-auto max-w-5xl px-6 py-12 sm:py-16">
      ...
      {data && data.summary && (
        <div className="mt-6 flex flex-wrap gap-x-8 gap-y-2 label-xs">
          ...
          <span className="text-primary">{data.summary.not_working} not working</span>
          <span className="text-foreground/40">{data.summary.not_tested} not tested</span>
        </div>
      )}
      ...
      <div className="mt-8 grid gap-3">
        {COMPONENTS.map(([key, label]) => {
          ...
          const tone = r.status === "WORKING" ? "text-sage" : r.status === "NOT_WORKING" ? "text-primary" : "text-foreground/40";
          return (
            <GlassCard key={key} className="flex items-center justify-between p-5">
              ...
              <div className="max-w-md text-right">
                ...
                <p className="mt-1 text-xs text-foreground/50">{r.detail}</p>
              </div>
            </GlassCard>
          );
        })}
      ...
      {data && data.error && <p className="mt-6 text-sm text-primary">{data.error}</p>}
    </div>
  );
}